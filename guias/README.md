...
Manual técnico maestro sobre la Fábrica de Golems interactiva en Parcela `[1, 2]` (`X: 35.0m, Z: 34.0m`). Detalla los activadores pointer in-world, la interfaz modal de mezclas React-ECS (`GolemForgeModal`), la memoización de derivación de recetas (`getMemoizedForgedGolem`), la validación estricta en tiempo real contra el catálogo oficial de 150 recetas (`src/data/recipesCatalog.ts`), el despliegue de miniaturas PNG render 3D del Golem, el pausado automático de animaciones 3D per-frame de engranajes y caldera (`factoryAnimationSystem`) cuando la modal está abierta para erradicar el lag en dispositivos móviles, los hitboxes táctiles ampliados a $50 \times 42\text{ px}$ para botones `+` y `-`, la cinemática automatizada de 3 planos con `VirtualCamera` e `InputModifier` (`playFactoryForgingCinematic`) y la regla de 1 único golem seguidor activo.
