...
| **19** | **Fábrica de Golems Interactiva, UI React-ECS, Cinemáticas y 150 Recetas** | Manual maestro de la fábrica interactiva en Parcela [1, 2] (35.0m, 34.0m), UI modal React-ECS de mezclas (5 a 12 componentes), validación estricta de las 150 recetas del GDD (`src/data/recipesCatalog.ts`), miniatura PNG render 3D del Golem, animación mecánica de engranajes/caldera (`factoryAnimationSystem`), cinemática 3D de 3 planos (`playFactoryForgingCinematic`) y regla de 1 golem seguidor activo. | [📖 Leer Guía](guia-fabrica-de-golems-interactiva-y-recetas-150.md) |
...
Manual técnico maestro sobre la Fábrica de Golems interactiva en Parcela `[1, 2]` (`X: 35.0m, Z: 34.0m`). Detalla los activadores pointer in-world, la interfaz modal de mezclas React-ECS (`GolemForgeModal`), la validación estricta en tiempo real contra el catálogo oficial de 150 recetas (`src/data/recipesCatalog.ts`), el despliegue de miniaturas PNG render 3D del Golem, el sistema de animación mecánica de engranajes y caldera (`factoryAnimationSystem`), la cinemática automatizada de 3 planos con `VirtualCamera` e `InputModifier` (`playFactoryForgingCinematic`) y la regla de 1 único golem seguidor activo.
